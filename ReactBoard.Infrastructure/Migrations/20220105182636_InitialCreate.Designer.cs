// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using ReactBoard.Infrastructure.DAL;

namespace ReactBoard.Infrastructure.Migrations
{
    [DbContext(typeof(DatabaseContext))]
    [Migration("20220105182636_InitialCreate")]
    partial class InitialCreate
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "3.1.22")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("ReactBoard.Domain.Entities.Board.Board", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("BoardUrlName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsWorkSafe")
                        .HasColumnType("bit");

                    b.Property<int>("MaxThreads")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Board");
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.Board.BoardAdminMapping", b =>
                {
                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<int>("BoardId")
                        .HasColumnType("int");

                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.HasKey("UserId", "BoardId");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("BoardAdminMapping");
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.Image.Image", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Image");
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.Image.ImageMetadata", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("Height")
                        .HasColumnType("int");

                    b.Property<long>("ImageId")
                        .HasColumnType("bigint");

                    b.Property<float>("Size")
                        .HasColumnType("real");

                    b.Property<int>("Width")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ImageId")
                        .IsUnique();

                    b.ToTable("ImageMetadata");
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.Post.Post", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long>("ThreadId")
                        .HasColumnType("bigint");

                    b.Property<int>("BoardId")
                        .HasColumnType("int");

                    b.Property<long?>("ImageId")
                        .HasColumnType("bigint");

                    b.Property<int?>("PostBoardId")
                        .HasColumnType("int");

                    b.Property<long?>("PostId")
                        .HasColumnType("bigint");

                    b.Property<long?>("PostThreadId")
                        .HasColumnType("bigint");

                    b.Property<string>("Text")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ThreadBoardId")
                        .HasColumnType("int");

                    b.Property<long>("ThreadId1")
                        .HasColumnType("bigint");

                    b.Property<DateTime>("Time")
                        .HasColumnType("datetime2");

                    b.HasKey("Id", "ThreadId", "BoardId");

                    b.HasIndex("ImageId")
                        .IsUnique()
                        .HasFilter("[ImageId] IS NOT NULL");

                    b.HasIndex("ThreadId1", "ThreadBoardId");

                    b.HasIndex("PostId", "PostThreadId", "PostBoardId");

                    b.ToTable("Post");
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.Thread.Thread", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("BoardId")
                        .HasColumnType("int");

                    b.Property<bool>("Locked")
                        .HasColumnType("bit");

                    b.HasKey("Id", "BoardId");

                    b.HasIndex("BoardId");

                    b.ToTable("Thread");
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.User.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("User");
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.User.UserRoleMapping", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("Role")
                        .HasColumnType("int");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("UserRoleMapping");
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.Board.BoardAdminMapping", b =>
                {
                    b.HasOne("ReactBoard.Domain.Entities.User.User", null)
                        .WithOne()
                        .HasForeignKey("ReactBoard.Domain.Entities.Board.BoardAdminMapping", "UserId")
                        .OnDelete(DeleteBehavior.ClientCascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.Image.ImageMetadata", b =>
                {
                    b.HasOne("ReactBoard.Domain.Entities.Image.Image", null)
                        .WithOne()
                        .HasForeignKey("ReactBoard.Domain.Entities.Image.ImageMetadata", "ImageId")
                        .OnDelete(DeleteBehavior.ClientCascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.Post.Post", b =>
                {
                    b.HasOne("ReactBoard.Domain.Entities.Image.Image", null)
                        .WithOne()
                        .HasForeignKey("ReactBoard.Domain.Entities.Post.Post", "ImageId")
                        .OnDelete(DeleteBehavior.ClientCascade);

                    b.HasOne("ReactBoard.Domain.Entities.Thread.Thread", null)
                        .WithMany("Posts")
                        .HasForeignKey("ThreadId1", "ThreadBoardId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ReactBoard.Domain.Entities.Post.Post", null)
                        .WithMany("Replies")
                        .HasForeignKey("PostId", "PostThreadId", "PostBoardId");
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.Thread.Thread", b =>
                {
                    b.HasOne("ReactBoard.Domain.Entities.Board.Board", null)
                        .WithMany("Threads")
                        .HasForeignKey("BoardId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ReactBoard.Domain.Entities.User.UserRoleMapping", b =>
                {
                    b.HasOne("ReactBoard.Domain.Entities.User.User", null)
                        .WithOne()
                        .HasForeignKey("ReactBoard.Domain.Entities.User.UserRoleMapping", "UserId")
                        .OnDelete(DeleteBehavior.ClientCascade)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
